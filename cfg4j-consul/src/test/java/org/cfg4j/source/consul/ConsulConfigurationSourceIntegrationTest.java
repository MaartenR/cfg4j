...
public class ConsulConfigurationSourceIntegrationTest {
      ...
      usWest2Toggle = !usWest2Toggle;
		...
		public MockResponse dispatch(RecordedRequest request) throws InterruptedException {
			...
			switch (request.getPath()) {
				...
				case "/v1/kv/us-west-2?recurse=true":
					String body = usWest2Toggle ? usWest2FeatureEnable : usWest2FeatureDisabled;
					return new MockResponse()
						.setResponseCode(200)
						.addHeader("Content-Type", "application/json; charset=utf-8")
						.setBody("[" + body + "]");
  ...
  public void getConfigurationShouldBeUpdatedByReload() throws Exception {
		Environment environment1 = new ImmutableEnvironment("us-west-2");
		assertThat(source.getConfiguration(environment1)).contains(MapEntry.entry("featureB.toggle", "disabled"));

    dispatcher.toggleUsWest2();
    source.reload();

    Environment environment2 = new ImmutableEnvironment("us-west-2");
		assertThat(source.getConfiguration(environment2)).contains(MapEntry.entry("featureB.toggle", "enabled"));
  }
